...
void driveControl() {
    int throttle = dThrot;  //FPS style drive controls
    int turning = dTurn;	//Taking inputs from input range of -128 to 127 and scaling them to -100 to 100
...
task main()
{
  ...
  for(int i = 0; i < 10; i++) {	//drive forward for a little bit, raise arm too
  	dThrot = 100;
  	driveControl();
	...
	for(int i = 0; i < 10; i++) {	//give arm a little time to settle at target
  	armPos = ARM_SCORE;
  	armControl();
  	wait1Msec(50);
	}
  ...
  for(int i = 0; i < 10; i++) {	//Close claw, drop wrist, in position for scoring.
  	clawState = false;
  	wristIn = -20;
  	manipulatorControl();
  	wait1Msec(50);
